...
def load_image_from_h5(path: str, img_size: int):
    with h5py.File(path, 'r') as f:
        image = f['depth'][:]  # (368, 1232, 3), float32, normalized [0, 1]
...
def load_image_from_h5_2(path: str):
    with h5py.File(path, 'r') as f:
        image = f['depth'][:]  # (368, 1232), unnormalized
    image_tensor = to_tensor(image) # (1, 368, 1232), unnormalized
    return image_tensor

class MixedBatchSampler(BatchSampler):
    """
    Mixed batch sampler, adapted from Marigold (https://github.com/prs-eth/Marigold)
    ...
    def _create_batch_samplers(self):
        ...
        if hasattr(self, "prob") and len(self.n_batches) == len(self.prob):
            ...
            # Set probability to 0 for datasets with no batches
            ...
            # Get probabilities for active set and normalize
            ...
            # When prob not provided: proportional to batch counts → normalize only on active set
    ...
    def __iter__(self):
        """
        Generator that yields batch indices - uses active set sampling to avoid index offset issues
        Returns:
        ...
        while yielded < self.n_total_batch:
            ...
            if len(self.raw_batches[idx_ds]) == 0:
                # Dataset exhausted: remove it from current active set
                active.pop(k)
                if len(active) == 0:
                    break  # Should not happen in theory, as n_total_batch is sum of active set
                
                # Renormalize probabilities for remaining active datasets
            ...
            # Apply offset using cumulative length in original order
...
# Note: Code copied from 'depth estimation', and for quick open-source release, dataset class names were not modified due to time constraints
class VKITTIDataset(data.Dataset):
    ...
    def __init__(self, dataset_dir: str, img_size: int = 768):
        ...
        condition_files = [f for f in os.listdir(conditions_dir) 
                          if not f.startswith('sintel')]
        
        for condition_file in condition_files:
            image_path = os.path.join(
                os.path.join(dataset_dir, "normal"), 
    ...
    def __getitem__(self, idx):
        ...
        sample["x_normal_ori"] = load_image_from_h5_2(item["depth_unnorm_path"])
...
class MixedDataModule:
    """
    Mixed data module implementing mixed sampling strategy similar to Marigold (https://github.com/prs-eth/Marigold)
    """
    def __init__(
        self, 
        dataset_dir: str, 
        ...
        img_size: int = 768, 
        # Note: Code copied from 'depth estimation', and for quick open-source release, dataset class names were not modified due to time constraints
        mixing_prob: List[float] = [0.015, 0.985],  # [Hypersim probability, VKITTI probability]
        seed: int = 0,
        num_workers: int = 1
    ):
        self.dataset_dir = dataset_dir
    ...
    def _create_datasets(self):
        ...
        return self._hypersim_dataset, self._vkitti_dataset

    def _prepare_datasets_and_probs(self):
        """Prepare dataset list and probability list"""
        hypersim_dataset, vkitti_dataset = self._create_datasets()
        ...
        if self._dataset_list is None:
            ...
            if len(vkitti_dataset) > 0:
                ...
                self._prob_list.append(self.mixing_prob[1] if len(hypersim_dataset) > 0 else 1.0)
