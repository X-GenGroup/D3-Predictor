...
def setup_distributed():
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        print("Distributed environment variables not detected, using single GPU training")
        rank = 0
        world_size = 1
        local_rank = 0
...
@hydra.main(config_path="configs", config_name="sd21_d3_predictor_normal", version_base="1.1")
def main(cfg: DictConfig):
    ...
    if mixed_precision_type == 'auto':
        # Auto-detect best precision type
    ...
    # Optional: if external deepspeed config file provided, use it to override built-in config
    ...
    if global_rank == 0:
        wandb.init(
            ...
            name=f"d3_predictor_normal_{time.strftime('%Y%m%d-%H%M%S')}",
            config=OmegaConf.to_container(cfg, resolve=True)
        )

    # Create data module, pass distributed parameters
    data = cfg.data
    # Set DataModule distributed parameters
    ...
    # Resume functionality: resume training from checkpoint
    start_step = 0
    if resume_from_checkpoint:
        ...
        if client_state:
            ...
            if global_rank == 0:
                print(f"✅ Successfully resumed to step {start_step}, epoch {i_epoch + 1}, Current LR: {lr_scheduler.get_last_lr()[0]}")
                if "gradient_loss" in client_state:
                    print(f"📊 Restored loss value: gradient_loss={client_state['gradient_loss']:.6f}")
        else:
            if global_rank == 0:
    ...
    step = start_step  # Start from resumed step
    loss_sum, align_loss_sum, mse_loss_sum, angular_loss_sum = 0.0, 0.0, 0.0, 0.0
    align_losses_sum = {}
    ...
    while not stop:  # Epochs
        ...
        for batch in (
            pbar := tqdm(dataloader_train, desc=f"Training (Epoch {i_epoch + 1})", disable=(global_rank != 0))
        ):  
            losses = model_engine(**dict_to(batch, device=model_engine.device))

            align_losses = {k: v for k, v in losses.items() if "sample" not in k}
            mse_sample = losses.get("mse_sample", torch.tensor(0.0, device=model_engine.device))
            angular_sample = losses.get("angular_sample", torch.tensor(0.0, device=model_engine.device))
            ...
            angular_loss_total = angular_sample.mean() if hasattr(angular_sample, 'mean') else angular_sample

            lambda_align = 1.0
            lambda_mse = 8.0
            lambda_angular = 3.0
            total_loss = lambda_align * align_loss_total + lambda_mse * mse_loss_total + lambda_angular * angular_loss_total
            ...
            angular_loss_sum += float(angular_loss_total.detach().item())
            ...
            if accum_count % grad_accum_steps == 0:
                ...
                avg_angular_loss = angular_loss_sum / grad_accum_steps
                ...
                if global_rank == 0:
                    ...
                    wandb.log({
                        ...
                        "Train/Angular Loss Total": avg_angular_loss,
