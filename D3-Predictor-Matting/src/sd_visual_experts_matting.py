...
class StableFeatureAligner(nn.Module):
    ...
    def multi_scale_gradient_loss(self, pred, target, scales=[1, 2, 4]):
        total_loss = 0
        for scale in scales:
            # Downsample
            ...
            total_loss += (loss_dx + loss_dy)
        
        return total_loss / len(scales)

    def forward(
        self, x_condition: Float[torch.Tensor, "b c h w"],         x_image: Float[torch.Tensor, "b c h w"], caption: list[str], **kwargs
        ...
        if self.alignment_loss == "cossim":
            ...
            losses["mse_sample"] = F.mse_loss(decoded_sample, x_image_expanded.detach())
            
            # Compute L1 loss
            losses["l1_sample"] = F.l1_loss(decoded_sample, x_image_expanded.detach())
            
            # Compute gradient loss (Gradient Loss)
            ...
            decoded_1ch_processed = decoded_sample_processed.mean(dim=1, keepdim=True)  # (B*N_T, 1, H, W), range [0, 1]
            pred_depth = decoded_1ch_processed  # shape: (B*N_T, 1, H, W), range [0,1]
            ...
            x_image_resized = torch.clamp(x_image_expanded.mean(dim=1, keepdim=True).detach() * 0.5 + 0.5, 0, 1)
            
            losses["gradient"] = self.multi_scale_gradient_loss(pred_depth, x_image_resized.detach())
