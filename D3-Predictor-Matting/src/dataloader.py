...
class MixedBatchSampler(BatchSampler):
    ...
    def _create_batch_samplers(self):
        ...
        if hasattr(self, "prob") and len(self.n_batches) == len(self.prob):
            ...
            # When prob is not provided: proportional to batch counts → normalize only on active set
    ...
    def __iter__(self):
        ...
        while yielded < self.n_total_batch:
            ...
            if len(self.raw_batches[idx_ds]) == 0:
                ...
                active.pop(k)
                if len(active) == 0:
                    break  # Should not happen in theory, as n_total_batch is the sum of active set
            ...
            # Apply offset using cumulative lengths in original order
...
# Note: Code copied from 'depth estimation', and due to time constraints for open-sourcing, dataset class names were not modified
class VKITTIDataset(data.Dataset):
    ...
    def __init__(self, dataset_dir: str, img_size: int = 768):
        ...
        conditions_dir = os.path.join(dataset_dir, "blurred_image")
        ...
        print(f"VKITTI condition file count: {len(condition_files)}")
        
        for condition_file in condition_files:
            image_path = os.path.join(
                os.path.join(dataset_dir, "mask"), 
                os.path.splitext(condition_file)[0] + ".png"
            )
    ...
    def __getitem__(self, idx):
        ...
        sample["x_image"] = load_image(item["image_path"], img_size=self.img_size)
        sample["caption"] = item["caption"]
        return sample

# Note: The second dataset is not used in the image matting task, so this class can be ignored
...
# The third dataset is not used in the image matting task, so this class can be ignored
class COCODataset(data.Dataset):
    ...
    def __init__(self, dataset_dir: str, img_size: int = 768):
        ...
        condition_files = [f for f in os.listdir(conditions_dir) 
                          if f.endswith('.jpg') and f.startswith('unsplash')]
...
class MixedDataModule:
    ...
    def __init__(
        ...
        batch_size: int = 1, 
        img_size: int = 768, 
        # Only one dataset is used in the image matting task
        mixing_prob: List[float] = [0, 1.0, 0],  # [Hypersim probability, VKITTI probability, COCO probability]
        seed: int = 0,
        num_workers: int = 1
    ):
        self.dataset_dir = dataset_dir
    ...
    def _prepare_datasets_and_probs(self):
        ...
        if self._dataset_list is None:
            ...
            print( f"VKITTI dataset size: {len(vkitti_dataset)}")
